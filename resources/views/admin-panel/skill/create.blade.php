            ...
            <div class="col-md-12">
                <div class="skill-create-form m-5 p-5 border bg-light">
                    <div class="shadow p-5 m-5">
                        <h4 class="mb-4" >Skill Creation Form</h4>
                        <hr>
                        <form action="{{ url('admin/skills') }}" method="POST" class="mt-5">
                            @csrf
                            <div class="form-group mb-4">
                                <label for="" class="form-label mb-2">Name</label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    name="name" placeholder="Enter Skill Name" value="{{ old('name') }}"
                                >
                                @error('name')
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control @error('percent') is-invalid @enderror"
                                    name="percent" placeholder="Enter Skill Percent with Number" value="{{ old('percent') }}"
                                >
                                @error('percent')
                                    <span><small class="text-danger">{{ $message }}</small></span>
                                @enderror
